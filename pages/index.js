...
const MAINTENANCE = [
  { icon: "🚗", title: "Programas Particulares", desc: "Mantenimiento personalizado según el uso y kilometraje exacto de tu auto. Tú solo conduce, nosotros planificamos el resto." },
  { icon: "🚛", title: "Paquetes para Flotillas", desc: "Los mejores planes corporativos. Reducción garantizada de costos operativos y minimización de tiempos muertos por fallas." },
  { icon: "📊", title: "Historial Digital Exacto", desc: "Sistema de control de mantenimiento que registra fechas, kilometrajes y servicios para prevenir averías futuras." },
];
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Diagnóstico computarizado avanzado y mantenimiento integral para flotillas y particulares. Matamoros, Tamaulipas." />
      ...
      <div className="page">
        ...
        <nav className={`navbar ${scrolled ? "scrolled" : ""}`} role="navigation" aria-label="Navegación principal">
          ...
          <button className="mobileMenuBtn" onClick={() => setMobileMenu(!mobileMenu)} aria-label={mobileMenu ? "Cerrar" : "Menú"} aria-expanded={mobileMenu}>{mobileMenu ? "✕" : "☰"}</button>
        </nav>
        ...
        <section className="trustSection" aria-label="¿Es para ti?">
          <div className="trustGrid">
            {[{ icon: "🔁", text: "Ya fuiste a otros talleres sin solución" }, { icon: "💸", text: "Cambiaste piezas y sigue fallando" }, { icon: "🎯", text: "Buscas un diagnóstico real de una vez" }].map((item, i) => (
          ...
          <div className="servicesGrid">
            {SERVICES.map((s, i) => (
              <div key={i} ref={setAnimRef} data-anim="up" style={{ transitionDelay: `${i * 0.1}s` }} className="serviceCard">
                <span className="serviceIcon">{s.icon}</span><h3 className="serviceTitle">{s.title}</h3><p className="serviceDesc">{s.desc}</p>
              </div>
            ))}
          </div>
        </section>

        {/* ═══ NUEVA SECCIÓN: LIDERAZGO EN MANTENIMIENTO ═══ */}
        <section className="section maintSection">
          <div ref={setAnimRef} data-anim="up" className="maintBadge">🏆 Referentes en Mantenimiento Integral</div>
          <h2 ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.1s" }} className="sectionTitle">Cuidamos todos los sistemas de tu vehículo</h2>
          <p ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.15s" }} className="sectionSubtitle">No solo reparamos, creamos programas de mantenimiento preventivo para particulares y las mejores flotillas de Matamoros.</p>
          
          <div className="maintGrid">
            {MAINTENANCE.map((m, i) => (
              <div key={i} ref={setAnimRef} data-anim="up" style={{ transitionDelay: `${i * 0.12}s` }} className="maintCard">
                <span className="maintIcon">{m.icon}</span>
                <h3 className="maintTitle">{m.title}</h3>
                <p className="maintDesc">{m.desc}</p>
              </div>
            ))}
          </div>

          <div ref={setAnimRef} data-anim="scale" style={{ transitionDelay: "0.3s" }} className="maintHighlight">
            <span className="maintHighlightIcon">🔒</span>
            <div>
              <h4 className="maintHighlightTitle">Control Total de tu Vehículo</h4>
              <p className="maintHighlightText">A todos nuestros clientes los integramos a nuestra <strong>base de datos profesional</strong>. Llevamos un registro exacto y preciso de kilometrajes, fechas y servicios para anticiparnos a cualquier falla y planificar tus mantenimientos a la perfección.</p>
            </div>
          </div>
        </section>
        ...
        <section id="contacto" className="section">
          ...
          <div className="contactGrid">
            <div ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.1s" }} className="contactCard">
              <span className="contactIcon">📍</span><h3 className="contactTitle">Ubicación</h3><p className="contactDetail">Calle 20 #1113, Col. Buena Vista<br />Matamoros, Tamaulipas</p>
            ...
            <div ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.2s" }} className="contactCard">
              <span className="contactIcon">💬</span><h3 className="contactTitle">WhatsApp</h3><p className="contactDetail">+52 868 390 5686<br />Respuesta rápida</p>
            ...
            <div ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.3s" }} className="contactCard">
              <span className="contactIcon">📧</span><h3 className="contactTitle">Correo electrónico</h3><p className="contactDetail">{EMAIL}</p>
            ...
            <div ref={setAnimRef} data-anim="up" style={{ transitionDelay: "0.4s" }} className="contactCard">
              <span className="contactIcon">🌐</span><h3 className="contactTitle">Sitio web</h3><p className="contactDetail">borderlineautomotivesolutions<br />.com</p>
      ...
      <style jsx global>{`
        :root {
          --bg: #050510; --bg2: #0d0a2e; --accent: #7c3aed; --accent-lt: #a78bfa; --accent-glow: rgba(124,58,237,0.4);
          ...
          --t1: #f1f1f7; --t2: #9898b0; --t3: #5c5c78; --r-sm: 8px; --r-md: 12px; --r-lg: 20px; --r-xl: 28px;
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .serviceCard:hover { border-color: var(--glass-bh); background: rgba(255,255,255,0.06); transform: translateY(-4px); box-shadow: 0 20px 40px rgba(0,0,0,0.3); } .serviceCard:hover::before { opacity: 1; }
        .serviceIcon { font-size: 36px; margin-bottom: 16px; display: block; } .serviceTitle { font-size: 18px; font-weight: 600; margin-bottom: 8px; } .serviceDesc { font-size: 14px; color: var(--t2); line-height: 1.6; }
        
        /* ═══ NUEVO CSS: MANTENIMIENTO ═══ */
        .maintSection { background: linear-gradient(180deg, transparent, rgba(0,230,118,0.02), transparent); }
        .maintBadge { display: inline-block; background: linear-gradient(135deg, rgba(0,230,118,0.1), rgba(0,200,83,0.05)); border: 1px solid rgba(0,230,118,0.2); color: var(--green); padding: 8px 20px; border-radius: 50px; font-size: 13px; font-weight: 600; margin: 0 auto 24px; text-align: center; width: fit-content; }
        .maintGrid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px; }
        .maintCard { background: var(--glass); border: 1px solid var(--glass-b); border-radius: var(--r-lg); padding: 32px 24px; text-align: center; transition: 0.3s ease; position: relative; overflow: hidden; }
        .maintCard::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, var(--green), transparent); opacity: 0; transition: opacity 0.4s ease; }
        .maintCard:hover { border-color: rgba(0,230,118,0.3); background: rgba(0,230,118,0.03); transform: translateY(-4px); box-shadow: 0 20px 40px rgba(0,0,0,0.3); } .maintCard:hover::after { opacity: 1; }
        .maintIcon { font-size: 40px; margin-bottom: 16px; display: block; }
        .maintTitle { font-size: 18px; font-weight: 600; margin-bottom: 10px; color: var(--t1); }
        .maintDesc { font-size: 14px; color: var(--t2); line-height: 1.6; }
        .maintHighlight { background: linear-gradient(90deg, rgba(0,230,118,0.06), rgba(59,130,246,0.06)); border: 1px solid rgba(0,230,118,0.15); border-radius: var(--r-md); padding: 28px; display: flex; align-items: flex-start; gap: 20px; }
        .maintHighlightIcon { font-size: 28px; flex-shrink: 0; margin-top: 2px; }
        .maintHighlightTitle { font-size: 17px; font-weight: 600; margin-bottom: 8px; color: var(--t1); }
        ...
        @media (max-width: 900px) {
          ...
          .maintGrid { grid-template-columns: 1fr 1fr; }
        }
        @media (max-width: 768px) {
          .section { padding: 60px 20px; }
          .navLinks { display: none; } .mobileMenuBtn { display: block; }
          ...
          .servicesGrid, .maintGrid { grid-template-columns: 1fr; max-width: 400px; margin-left: auto; margin-right: auto; margin-bottom: 30px; }
        ...
        @media (prefers-reduced-motion: reduce) {
          *, *::before, *::after { animation-duration: 0.01ms !important; animation-iteration-count: 1 !important; transition-duration: 0.01ms !important; }
          [data-anim="up"], [data-anim="scale"] { opacity: 1; transform: none; } .blob { display: none; }
        }
      `}</style>
    </>
  );
